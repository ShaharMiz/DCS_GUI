...
//                  Variables
//------------------------------------------------
volatile unsigned int Vx=origin; // the middle - 460
volatile unsigned int Vy=origin; // the middle - 460
...
//                      PB1 ISR
//==========================================================
//----------- Port1 Interrupt Service Routine - PB1 --------
...
volatile int StepperDelay = 2;           // max freq, f = MHz
...
void write_seg (char* flash_ptr){
    ...
    FCTL1 = FWKEY + ERASE;                      // Set ERASE bit for delete operation
    *flash_ptr = 0;                             // write in deleted memory

    FCTL1 = FWKEY + WRT;                        // Set WRT bit for write operation
    int n;
    for (n=0; n<script_size_counter; n++ ){
        *flash_ptr++ = to_flash[n];             // Write value to flash
    }
...
__interrupt void USCI0RX_ISR(void)
{
    ...
    if (first_byte_MSG == '#'){         // '#'- Ask for status (#- in hex-35)
        ...
        // Get status_stage value for calibration or moving freely
        ...
        // Get Scripts information
            ...
            Msg_location = 0;
            if(write_to_flash){             // Get Script chars to write in flash
                to_flash[script_idx++] =  RxBuffer;
            }else if (ScriptNumFlg==1){     // Get Script number
                ScriptNumFlg = 0;
                ScriptNum = RxBuffer;
            }else if (ScriptNumFlg==0){     // Get Script number of lines and bytes number
                p_rx[index++] = RxBuffer;
            }
